...
begin
  ...
  PU.Send_Line ("USART INICIALIZADO");
   Wait (100);
  PU.Send_Line ("I2C_CONFIG:");
   Bus.Mostrar_Config;
   --Bus.Test_Hardware;
   --Bus.Scan_I2C_Bus;
   --Bus.Test_Minimo;
   Bus.Initialize;
   PU.Send_Line ("I2C INICIALIZADO");
   ...
   begin
      if (SR2 and Uint32 (2)) /= 0 then
         PU.Send_Line ("DIAG: bus BUSY!");
      else
         PU.Send_Line ("DIAG: bus libre OK");
      end if;
      if (CR1 and Uint32 (2**I2C_CR1_PE)) /= 0 then
        PU.Send_Line ("DIAG: PE=1 OK");
      else
         PU.Send_Line ("DIAG: PE=0 PROBLEMA");
      end if;
   end;

   Wait (100);
   SSD1306.Init;
 PU.Send_Line ("SSD1306 INICIALIZADO");
   Wait (1500);

   Test_Text;             PU.Send_Line ("Test_Text OK");
   Test_Pixels;           PU.Send_Line ("Test_Pixels OK");
   Test_Lines;          PU.Send_Line ("Test_Lines OK");
   Test_Rectangles;      PU.Send_Line ("Test_Rectangles OK");
   Test_Filled_Rectangles; PU.Send_Line ("Test_Filled OK");
   Test_Effects;        PU.Send_Line ("Test_Effects OK");
   Test_Animation;      PU.Send_Line ("Test_Animation OK");
   Test_Full_Demo;        PU.Send_Line ("Test_Full_Demo OK");
